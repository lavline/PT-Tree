...
void single_thread_cycle(vector<uint8_t> set_field, int set_port, bool enable_log, int log_level, bool enable_update, vector<Rule>& rules, vector<Packet>& packets, vector<int>& check_list)
{
	...
	fclose(res_fp);
	cout << "|- Average search Cycle: " << search_cycle / packets.size() << "\n";
	...
	if (enable_update) {
		int update_num = 5000;
		cout << "\nStart update...\n";
		uint64_t total_cyl = 0;
		bool _u = tree.update_cycle(rules, update_num, total_cyl);
		if (_u) {
			cout << "|- Average update Cycle: " << total_cyl / (update_num * 2.0) << "\n";
		}
	}
}
...
void multi_thread_cycle(vector<uint8_t> set_field, int set_port, bool enable_log, int log_level, bool enable_update, vector<Rule>& rules, vector<Packet>& packets, vector<int>& check_list)
{
	...
	for (; thread_num <= 32; thread_num *= 2) {
		...
		for (int i = 0; i < thread_num; ++i) {
			threads[i].join();
			total_throughput += avg_cycle[i];
		}
		tree.free_del_leaf();
		printf("\n%d thread avg cycle with update: %f\n", thread_num, total_throughput / thread_num);
